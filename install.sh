...
if ! curl --location --output "${tarball}" \
	"https://downloads.raspberrypi.org/raspios_${arch}/root.tar.xz"; then
...
proot --link2symlink tar -xf "${tarball}" --directory="${HOME}/.${directory}/rootfs" --exclude='dev' > /dev/null 2>&1
rm -f "${tarball}"
